...
function Pill({ children }: { children: React.ReactNode }) {
  return (
    <span className="px-2 py-0.5 rounded bg-zinc-800 border border-zinc-700 text-gray-100 text-xs">
      {children}
    </span>
  );
}
...
export const columns: ColumnDef<Pedido>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return p ? <Pill>{prioridadeLabel[p]}</Pill> : "—";
    },
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="flex items-center gap-2 justify-center">
          {/* Botão Ver */}
          <button
            className="flex items-center bg-gray-500 px-3 py-1 rounded-xl text-gray-100 hover:bg-zinc-800 cursor-pointer"
            onClick={() => console.log("Ver pedido:", pedido.id)}
          >
            <IconEye className="w-4 h-4 mr-1" /> Ver
          </button>

          {/* Botão Finalizar com modal */}
          <AlertDialog>
            <AlertDialogTrigger asChild>
              <button className="flex items-center bg-green-600 px-3 py-1 rounded-xl text-white hover:bg-green-700 cursor-pointer">
                ✓ Finalizar
              </button>
            </AlertDialogTrigger>
            <AlertDialogContent className="bg-zinc-900 border border-zinc-700">
              ...
              <AlertDialogFooter>
                <AlertDialogCancel className="bg-zinc-800 text-gray-100 border-zinc-600 hover:bg-zinc-700">
                  Cancelar
                </AlertDialogCancel>
                <AlertDialogAction
                  className="bg-green-600 hover:bg-green-700 text-white"
                  onClick={() =>
                    console.log("Confirmar finalização do pedido:", pedido.id)
                  }
                >
                  Confirmar
                </AlertDialogAction>
          ...
          <Sheet>
            ...
            >
              ...
              <div className="space-y-4 overflow-auto max-h-[calc(100vh-150px)] pr-1">
                ...
                {/* Situação */}
                <div className="flex flex-col gap-1">
                  <label className="text-xs text-gray-400">Situação</label>
                  <select
                  ...
                  table.getRowModel().rows.map((row) => (
                    <TableRow key={row.id}>
                      {row.getVisibleCells().map((cell) => (
                        <TableCell key={cell.id} className="py-3 text-gray-300">
                          {flexRender(
